...
def bi_lstm_model(sequence_length, embedding_dim):
...
def train_BiLSTM(X, y, model, inp_dim, weights, epochs=EPOCHS, batch_size=BATCH_SIZE):
...
if __name__ == "__main__":
    ...
    model = bi_lstm_model(data.shape[1], EMBEDDING_DIM)
    #model = lstm_model(data.shape[1], 25, get_embedding_weights())
    train_BiLSTM(data, y, model, EMBEDDING_DIM, W, EPOCHS, BATCH_SIZE)
    print( "Saving model...")
    model.save('bi_lstm_' + INITIALIZE_WEIGHTS_WITH + '.h5')