...
def select_tweets():
    ...
    #tweet_return_file = "bi_lstm_ws_e4_tweets.pickle"
...
def gen_vocab():
    ...
    #with hashtag splitting(WS)
    #vocab_file = "bi_lstm_ws_e4_vocab.pickle"
    #reverse_vocab_file = "bi_lstm_ws_e4_reverse_vocab.pickle"
...
def gen_sequence():
    ...
    #X_file = "bi_lstm_ws_e4_X.pickle"
    #y_file = "bi_lstm_ws_e4_y.pickle"
...
def cnn_gru_model(sequence_length, embedding_dim):
    ...
    model.add(Convolution1D(100, FILTER_SZ, border_mode='valid', activation='relu'))   
    model.add(MaxPooling1D(pool_length=4,border_mode='valid'))  #pool_length in keras 1.2.2 not pool_size   
    model.add(GRU(50))
...
def train_CNN_GRU(X, y, inp_dim, model, weights, epochs=EPOCHS, batch_size=BATCH_SIZE):
...
if __name__ == "__main__":
    ...
    model = cnn_gru_model(data.shape[1], EMBEDDING_DIM)
    train_CNN_GRU(data, y, EMBEDDING_DIM, model, W, EPOCHS)
    print("Saving model...")
    model.save('cnn_gru_' + INITIALIZE_WEIGHTS_WITH + '.h5')